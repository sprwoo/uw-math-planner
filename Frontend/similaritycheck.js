...
/** 
 * Adds to the major's name as a value into the object with the course code as the key
 * @param {Object} object   The object to add the key and name to
 * @param {String} key      The name of the course to be added as a key
 * @param {String} name     The name of the major to be added as a value
 * @returns {void}
*/
function add_to_dict(object, key, name) {
    // Get rid of whitespaces and single quotes
    key = key.trim();
    key = key.slice(1, -1);

    // Check if the key is a black space
    if (key.length > 0) {
        // If it alreaady exists, push it into the array, otherwise initalize the array
...
/**
 * Adds the major's requirements to an object
 * @param  {Object} major   The object representing the major
 * @param  {Object} object  The object to add the requirements to
 * @return {void}
 */
function parse_csv_to_obj(major, object) {
    let values = false;
    let key = "";
    ...
    for (let i = 0; i < major.Requirements.length; i++) {
        ...
        if (ch == '[') {
            ...
            add_to_dict(object, key, major.Major);
            values = false;
            key = "";
        } else if (ch == ',' && values) {
            add_to_dict(object, key, major.Major);
